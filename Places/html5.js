...
FlipBook.prototype = {
	...
	,removeBookmark: function(pageNum) {
		...
		while(_g1 < _g) {
			var i1 = _g1++;
			haxe.Log.trace(RunTime.book.bookmarks[i1].pageNum,{ fileName : "FlipBook.hx", lineNumber : 1488, className : "FlipBook", methodName : "removeBookmark"});
	...
	,turnPage: function(pageOffset) {
		...
		this.tweener.onChange = function(count) {
			...
			if(RunTime.book.rightToLeft) self.bookContext.pageOffset = pageOffset * ratio * ratio * ratio; else self.bookContext.pageOffset = -pageOffset * ratio * ratio * ratio;
...
RunTime.requestBookInfo = function() {
	orc.utils.Util.request(RunTime.urlBookinfo,function(data) {
		...
		if(encode == RunTime.book.password || RunTime.book.lockPages.length > 0) {
...
core.ButtonInfo.prototype = {
	...
	,loadToRect: function(ctx,x,y,w,h) {
		if(w > 0 && h > 0) ctx.drawImage(this._imagePage,0,0,this._imagePage.width,this._imagePage.height,x,y,w,h);
	}
