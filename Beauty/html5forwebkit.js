...
FlipBook.prototype = {
	...
	,removeBookmark: function(pageNum) {
		...
		while(_g1 < _g) {
			var i1 = _g1++;
			haxe.Log.trace(RunTime.book.bookmarks[i1].pageNum,{ fileName : "FlipBook.hx", lineNumber : 1723, className : "FlipBook", methodName : "removeBookmark"});
	...
	,getZoomOutCSS: function() {
		return this.zoomCSS + "-webkit-transform:scale(" + 1 + " );-webkit-transition: all 0ms ;" + "-moz-transform:scale(" + 1 + ");-moz-transition: all 0ms ;" + "top:" + 0 + "px;left:" + 0 + "px;";
	}
...
RunTime.loadBookInfo = function() {
	...
	var menuArray = [RunTime.flipBook.btnContents,RunTime.flipBook.btnThumbs,RunTime.flipBook.btnSearch,RunTime.flipBook.btnShowTxt,RunTime.flipBook.btnMask,RunTime.flipBook.btnBookMark,RunTime.flipBook.btnAutoFlip,RunTime.flipBook.btnEmail,RunTime.flipBook.btnSns];
	var showArray = [RunTime.book.menuTocVisible,RunTime.book.menuThumbsVisible,RunTime.book.menuSearchVisible,RunTime.book.menuTxtVisible,RunTime.book.menuHighlightVisible,RunTime.book.menuBookmarkVisible,RunTime.book.menuAutoFlipVisible,RunTime.book.menuEmailVisible,RunTime.book.menuSnsVisible];
	var menuCount = 9;
	var t = js.Lib.window.navigator.userAgent;
	if(t.indexOf("CPU iPhone OS ") != -1) {
		var iosVar = HxOverrides.substr(t,t.indexOf("CPU iPhone OS ") + 14,1);
	...
	while(_g1 < _g) {
		...
		if(szKey.indexOf(RunTime.kvPrex) == 0) {
			if(szKey.indexOf("@$bm$@") != -1) {
				...
				haxe.Log.trace("bookmark.text:" + bookmark.text + "  pagenum: " + bookmark.pageNum,{ fileName : "RunTime.hx", lineNumber : 1939, className : "RunTime", methodName : "readLocalBookmarks"});
			}
		}
	}
	return bookmarks;
...
core.HtmlHelper.toPopupHtml = function(item) {
	...
	s += "<div style=\" height:" + Std.string(h - 24) + "px; line-height:120%; background-color:#333; margin:6px; padding:6px;\">";
